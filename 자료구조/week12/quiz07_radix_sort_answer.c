#include <stdio.h>

#define BUCKETS 10
#define MAX_QUEUE_SIZE 100

typedef int element;

typedef struct {
    element data[MAX_QUEUE_SIZE];
    int front;
    int rear;
} QueueType;

static void init(QueueType *q)
{
    q->front = 0;
    q->rear = 0;
}

static int is_empty(QueueType *q)
{
    return q->front == q->rear;
}

static void enqueue(QueueType *q, element item)
{
    q->rear = (q->rear + 1) % MAX_QUEUE_SIZE;
    q->data[q->rear] = item;
}

static element dequeue(QueueType *q)
{
    q->front = (q->front + 1) % MAX_QUEUE_SIZE;
    return q->data[q->front];
}

static void print_array(const int list[], int n)
{
    for (int i = 0; i < n; i++) {
        printf("%d ", list[i]);
    }
    printf("\n");
}

static void radix_sort_trace(int list[], int n)
{
    QueueType queues[BUCKETS];
    int factor = 1;

    for (int pass = 1; pass <= 2; pass++) {
        for (int b = 0; b < BUCKETS; b++) {
            init(&queues[b]);
        }
        for (int i = 0; i < n; i++) {
            enqueue(&queues[(list[i] / factor) % 10], list[i]);
        }
        int i = 0;
        for (int b = 0; b < BUCKETS; b++) {
            while (!is_empty(&queues[b])) {
                list[i++] = dequeue(&queues[b]);
            }
        }
        printf("pass %d: ", pass);
        print_array(list, n);
        factor *= 10;
    }
}

int main(void)
{
    int list[] = { 52, 87, 42, 77, 53, 47, 85, 72 };
    int n = (int)(sizeof(list) / sizeof(list[0]));

    printf("start : ");
    print_array(list, n);
    radix_sort_trace(list, n);
    printf("resident registration numbers are fixed-length decimal keys, so radix sort O(dn) is attractive.\n");
    printf("quick sort is also good generally, but has comparison cost O(n log n).\n");
    return 0;
}
