...
static void resize_queue(QueueType *q)
{
    ...
    element *temp = (element *)malloc(sizeof(element) * q->size);
    element *new_data;
    ...
    new_data = (element *)realloc(q->data, sizeof(element) * new_capacity);
    if (new_data == NULL) {
        free(temp);
...
static void print_operations(const int operations[], int n)
{
    printf("Operations: ");
    for (int i = 0; i < n; i++) {
...
int main(void)
{
    ...
    for (int i = 0; i < n; i++) {
        if (operations[i] > 0) {
            ...
            printf("enqueue %d, queue = ", operations[i]);
            print_queue(&q);
            printf(" (capacity = %d)\n", q.capacity);
        } else if (operations[i] == -1) {
            int item = dequeue(&q);

            printf("dequeue -> %d, queue = ", item);
            print_queue(&q);
            printf(" (capacity = %d)\n", q.capacity);
        }
    }
